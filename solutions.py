...
def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
    ...
    # zero chance of the ghost not in the jail if
    # observation is not None; So we are confident to
    # say that whenever when ghost is not jail location but 
    # returned a observation of None
    # then return the probability of 0 meaning the the ghost 
    # cannot be in jail    
    if noisyDistance == None:
        return 0
...
def elapseTime(self, gameState):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Get the old ghost position
    for oldGhostPosition in self.allPositions:
        # Get new ghost position distribution (data)
        newPosDist = self.getPositionDistribution(gameState, oldGhostPosition)
        # obtain probability of ghost being in this new location 
        NewPosProb = newPosDist[oldGhostPosition]
        # obtain new ghost position
        for newPos in newPosDist.keys():
            # update probability on new position by multiplying
            # old porb with new porb
            self.beliefs[newPos] = NewPosProb * self.beliefs[oldGhostPosition]
